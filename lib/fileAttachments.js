...
function extractFileAttachments(snapshot) {
  return _.flatten(getAllComments(snapshot).map(comment =>
    _.map(comment.file_attachments, munge)
  ));
}
...
const fileAttachments = function(state = {}, action) {
  switch (action.type) {
    case "RESET_PROJECT":
      return _.indexBy(extractFileAttachments(action.project), 'id');
    case "LOAD_PAST_DONE_STORIES":
      ...
      let fileAttachmentsById = _.indexBy(fileAttachments, 'id');
...
function processCommand(state, result) {
  if (result.type === 'file_attachment') {
    ...
    let newAttachment = {
      ...state[result.id],
      ..._.omit(result, 'type', 'comment_id', 'height', 'width')
    };
