...
function munge(comment) {
  ...
  munged.google_attachment_ids = _.pluck(comment.google_attachments, 'id');
  munged.file_attachment_ids = _.pluck(comment.file_attachments, 'id');
  return munged;
}
...
const comments = function(state = {}, action) {
  switch (action.type) {
    case "RESET_PROJECT":
      return _.indexBy(extractComments(action.project), 'id');
    case "LOAD_PAST_DONE_STORIES":
      let storyComments = _.flatten(_.map(action.stories, 'comments')).map(munge);
      let storyCommentsById = _.indexBy(storyComments, 'id');
