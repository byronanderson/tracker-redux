...
const stories = function(state = {}, action, memberships) {
  switch (action.type) {
    case "RESET_PROJECT":
      ...
      return _.indexBy(stories, 'id');
    case "LOAD_PAST_DONE_STORIES":
      let newStoryData = _.indexBy(action.stories.map(munge), 'id');
      return {
        ...state,
        ...newStoryData,
      };
...
function processCommand(state, result, memberships) {
  if (result.type === 'story') {
    ...
    if (result.comments) {
      newStory.comment_ids = _.pluck(result.comments, 'id');
    }

    if (result.tasks) {
      newStory.task_ids = _.pluck(result.tasks, 'id');
    }
