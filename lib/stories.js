...
function munge(story) {
  let munged = _.omit(story, 'labels', 'project_id', 'comments', 'tasks', 'owned_by_id', 'blockers', 'blocked_story_ids');
...
function processCommand(state, result, memberships) {
  if (result.type === 'story') {
    ...
    let newStory = {
      ...state[result.id],
      ..._.omit(result, 'type', 'project_id', 'labels', 'before_id', 'after_id', 'tasks', 'comments', 'owned_by_id', 'blockers', 'blocked_story_ids'),
    };
  ...
  if (result.type === 'comment' && result.story_id) {
    ...
    if (story && !_.includes(story.comment_ids, result.id)) {
      return {
        ...state,
        [story.id]: {
          ...story,
  ...
  if (result.type === 'comment' && result.deleted) {
    let story = _.find(state, story => _.includes(story.comment_ids, result.id));
    if (story) {
      return {
        ...state,
        [story.id]: {
  ...
  if (result.type === 'task') {
    if (result.story_id) {
      ...
      let story = _.find(state, story => _.includes(story.task_ids, result.id));
      if (story) {
        if (result.deleted) {
          return {
            ...state,
